...
2. On **Settings**, under **Appearance**, choose **Dark** or **Light** (saved as `ui_theme` in `config.json`; default is **light**). Use the **繁中 / English** control on the same page to switch the UI language. Optionally enable **Work hours (09:00–18:00)** so nudges run only on **Mon–Fri** between those times in **local time**; outside that window and on **weekends** the app stays paused when running and shows a **schedule paused** countdown until the next allowed period (`schedule_window` in `config.json`). On **Windows** with the system tray available, you can turn on **Start with Windows** for a per-user autostart entry and **Minimize to the system tray when closing the window** as described later.
