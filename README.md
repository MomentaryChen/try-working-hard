...
8. Optional: enable **Work hours (09:00–18:00)** so the schedule runs only on **Mon–Fri** between those times in **local time**; outside that window and on **weekends** the app stays paused and shows a **schedule paused** countdown until the next allowed period (stored as `schedule_window` in `config.json`).
