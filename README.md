...
11. On **Settings**, use **Open config file** to open `config.json` (under `%APPDATA%\try-working-hard\` on Windows, or `~/.try-working-hard/` if `APPDATA` is unset) in the default application; if the file does not exist yet, the app writes the current settings first. The same page also includes **Contact us**, **Check for updates**, and **Auto-check updates on startup** for release awareness.
