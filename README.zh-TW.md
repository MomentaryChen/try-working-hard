...
6. 在「位移（像素）」輸入 **0–500** 的整數；意義依**移動路徑**而定：直線＝水平距離、圓＝半徑、方＝邊長。**0** 表示該次不位移。預設 **100**。
7. 在「**路徑速度**」輸入 **1–10** 的整數：直線、圓形、方形**繪製**的快慢（數字愈大愈快）。預設 **5**，存於 `config.json` 的 `path_speed_text`。
8. 選擇 **移動路徑**：**直線**、**圓形** 或 **方形**；偏好儲存在 `config.json` 的 `motion_pattern`。視窗高度不足時，首頁控制區可**捲動**。
