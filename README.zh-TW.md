...
2. 在 **設定** 頁，**設定** 標題固定在頂部，下方選項可獨立捲動。在 **外觀** 選擇 **深色** 或 **淺色**（存於 `config.json` 的 `ui_theme`；預設 **light**）。同一頁的 **繁中 / English** 可切換介面語言。在 **Windows** 且已安裝系統匣支援時，可開啟 **隨 Windows 啟動**，於目前使用者登錄的 **執行** 項目寫入登錄值，讓程式登入時以**系統匣**方式啟動。
