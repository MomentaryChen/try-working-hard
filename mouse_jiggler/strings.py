"""UI copy for zh / en."""

from __future__ import annotations

from typing import Literal

Lang = Literal["zh", "en"]

STRINGS: dict[Lang, dict[str, str]] = {
    "zh": {
        "window_title": "二次努力",
        "app_subtitle": "try-working-hard",
        "seg_control": "控制面板",
        "seg_log": "紀錄",
        "theme_appearance": "外觀",
        "theme_appearance_dark": "深色",
        "theme_appearance_light": "淺色",
        "theme_status_dark": "深色主題",
        "theme_status_light": "淺色主題",
        "lang_ui": "介面語言",
        "dashboard": "主控台",
        "interval_label": "間隔（秒）",
        "interval_presets_caption": "快速間隔",
        "interval_preset_10s": "10s",
        "interval_preset_30s": "30s",
        "interval_preset_1m": "1m",
        "interval_preset_5m": "5m",
        "autostart_switch_title": "隨 Windows 啟動",
        "autostart_switch_hint": "登入 Windows 時自動執行，並在系統匣中啟動。",
        "autostart_not_windows": "（僅支援 Windows。）",
        "autostart_requires_tray": "（需安裝 pystray 系統匣。）",
        "log_tray_start_hidden": "以系統匣模式執行（主視窗已隱藏）。",
        "interval_unit_min": "分鐘",
        "interval_unit_sec": "秒",
        "interval_hint_min": "≥ 0.1，可小數（鍵盤／滑鼠無輸入須達此時間）",
        "interval_hint_sec": "5–600，可小數（鍵盤／滑鼠無輸入須達此時間）",
        "interval_jitter_label": "間隔隨機（± 秒）",
        "interval_jitter_hint": "0 ＝ 關閉；每次閒置間隔為基準 ± 此值（秒）· 0–{max}",
        "motion_pattern_label": "移動路徑",
        "motion_pattern_label_when_natural": "移動路徑（僅路徑模式）",
        "motion_pattern_line": "直線",
        "motion_pattern_circle": "圓形",
        "motion_pattern_square": "方形",
        "activity_style_label": "優先選擇自然還是固定模式",
        "activity_style_pattern": "固定模式",
        "activity_style_natural": "自然模式",
        "activity_mode_params_fixed": "可設定參數：間隔、間隔隨機、位移、路徑速度、移動持續時間、移動路徑。",
        "activity_mode_params_natural": "可設定參數：間隔、間隔隨機、位移、微動速度、移動持續時間、偶發左鍵點擊、偶發滾輪捲動。",
        "fixed_params_title": "進階調整",
        "extra_params_title": "額外參數",
        "extra_params_expand": "展開額外參數",
        "extra_params_collapse": "收起額外參數",
        "natural_preset_label": "自然模式 Preset",
        "natural_preset_conservative": "保守",
        "natural_preset_standard": "標準",
        "natural_preset_aggressive": "激進",
        "natural_preset_load": "載入",
        "natural_preset_save": "儲存",
        "natural_preset_hint": "選擇 Preset 後可一鍵載入；「儲存」會覆寫目前所選 Preset。",
        "natural_rare_click": "偶發左鍵點擊（低機率）",
        "natural_rare_scroll": "偶發滾輪捲動（低機率）",
        "natural_intensity_label": "自然模式強度",
        "natural_intensity_conservative": "保守",
        "natural_intensity_standard": "標準",
        "natural_intensity_strong": "明顯",
        "natural_intensity_tooltip_title": "自然模式強度參數",
        "natural_intensity_tooltip_body": "保守：位移覆蓋 35–65%、步數 96–210、平滑 0.90、子步進 5。\n標準：位移覆蓋 70–95%、步數 60–140、平滑 0.86、子步進 3。\n明顯：位移覆蓋 95–100%、步數 34–82、平滑 0.82、子步進 3。",
        "natural_opts_hint": "僅在自然模式下套用；每次觸發至多一種額外動作，機率低。",
        "motion_pattern_log_natural": "自然",
        "pixels_label": "位移（像素）",
        "pixels_hint": "路徑模式：直線＝左右還原；圓＝半徑；方＝邊長。自然模式：游標偏離起點的最大距離 · {lo}–{hi}",
        "path_speed_label": "路徑速度",
        "path_speed_hint": "幾何路徑繪製快慢 · {lo}–{hi}（數字愈大愈快）",
        "path_speed_label_natural": "微動速度",
        "path_speed_hint_natural": "自然模式各微動步驟的間隔快慢 · {lo}–{hi}（數字愈大愈快）",
        "motion_duration_label": "移動持續時間（秒）",
        "motion_duration_hint": "每次微動的持續時間（秒） · {lo}–{hi}（最大 60 秒）",
        "log_start_fail_motion_duration": "開始失敗：移動持續時間無效。",
        "err_motion_duration": "請輸入有效的移動持續時間秒數（整數 {lo}–{hi}）。",
        "log_start_fail_path_speed": "開始失敗：路徑速度無效。",
        "err_path_speed": "請輸入有效路徑速度（整數，{lo}–{hi}）。",
        "status_motion_burst": "狀態：執行中 · 微動中（{md} 秒）· 剩餘約 {cd}",
        "btn_start": "開始",
        "btn_stop": "暫停",
        "tray_checkbox": "關閉視窗時縮到系統匣（排程繼續）",
        "tray_switch_title": "關閉視窗時縮到系統匣",
        "tray_switch_hint": "啟用後關閉視窗將保留排程並在背景執行，可從系統匣圖示還原視窗。",
        "schedule_window_title": "上班時段（週一至週五）",
        "schedule_window_start_label": "開始",
        "schedule_window_end_label": "結束",
        "schedule_window_hint": "本機 24 小時制 HH:MM；支援多段（用逗號分隔，如 09:00-12:00,13:00-18:00,21:00-23:00），預設平日；可於 config 啟用週末與 cron-like。",
        "schedule_banner_active": "排程時段：{start}–{end}（本機；結束不含）· 週末：{weekend} · cron：{cron}",
        "schedule_banner_need_valid_times": "排程已開啟：請設定有效時段（HH:MM-HH:MM，可多段），且各段不可重疊。",
        "err_schedule_bounds": "請輸入有效排程：多段 HH:MM-HH:MM（可逗號分隔、不重疊）；cron-like 欄位若有值也需合法。",
        "log_start_fail_schedule_bounds": "開始失敗：上班時段時間無效。",
        "status_schedule_wait": "狀態：排程暫停 · 下次上班時段約 {cd}",
        "log_start_schedule": "已啟用排程：基礎時段 {start}–{end}（本機）· 週末：{weekend} · cron：{cron}。",
        "log_schedule_resumed": "已回到上班時段內，排程續行。",
        "schedule_weekend_on": "啟用",
        "schedule_weekend_off": "停用",
        "schedule_cron_off": "未設定",
        "tray_no_pystray": "（未安裝 pystray）",
        "status_stopped": "狀態：已停止",
        "status_running_min": "狀態：執行中 · 鍵盤／滑鼠無輸入達 {v:g} 分鐘後微動 · 下次約 {cd}",
        "status_running_sec": "狀態：執行中 · 間隔 {v:g} 秒 · 移動持續 {md} 秒 · 下次約 {cd}",
        "log_title": "活動紀錄",
        "log_ready": "程式已就緒。",
        "log_start_fail_interval": "開始失敗：間隔設定無效。",
        "log_start_fail_pixels": "開始失敗：位移設定無效。",
        "log_started_min": "已開始，鍵盤／滑鼠無輸入達 {v:g} 分鐘（約 {sec:.0f} 秒）後微動、路徑 {pat}、位移 {px} px、路徑速度 {ps}、移動持續 {md} 秒。",
        "log_started_min_jitter": "已開始，鍵盤／滑鼠無輸入約 {v:g} 分鐘（基準 ~{sec:.0f} 秒）、±{j:g} 秒隨機間隔、路徑 {pat}、位移 {px} px、路徑速度 {ps}、移動持續 {md} 秒。",
        "log_started_sec": "已開始，鍵盤／滑鼠無輸入達 {v:g} 秒後微動、路徑 {pat}、位移 {px} px、路徑速度 {ps}、移動持續 {md} 秒。",
        "log_started_sec_jitter": "已開始，鍵盤／滑鼠無輸入約 {v:g} 秒（基準）、±{j:g} 秒隨機間隔、路徑 {pat}、位移 {px} px、路徑速度 {ps}、移動持續 {md} 秒。",
        "log_stopped": "已手動停止。",
        "log_nudge": "已執行游標微動。",
        "log_nudge_zero": "已觸發排程（位移 0，未移動游標）。",
        "log_nudge_fail": "微動失敗：{err}",
        "log_exit": "結束程式。",
        "log_tray_minimize": "已縮至系統匣（排程仍執行，可從圖示還原或結束）。",
        "err_title": "輸入錯誤",
        "err_minutes": "請輸入有效的分鐘數（數字，且 ≥ {min}）。",
        "err_seconds": "請輸入有效的秒數（數字，{min}–{max}）。",
        "err_jitter": "請輸入有效的隨機秒數（數字，0–{max}）。",
        "log_start_fail_jitter": "開始失敗：間隔隨機設定無效。",
        "err_pixels": "請輸入有效的位移像素（整數，{lo}–{hi}）。",
        "tray_show": "顯示主視窗",
        "tray_quit": "結束程式",
        "nav_home": "首頁",
        "nav_settings": "設定",
        "nav_analytics": "分析",
        "settings_title": "設定",
        "btn_open_config_file": "開啟設定檔",
        "about_updates_title": "關於與更新",
        "version_info": "版本：{version}",
        "btn_contact_us": "聯絡我們",
        "btn_check_updates": "檢查更新",
        "auto_check_updates_title": "啟動時自動檢查更新",
        "auto_check_updates_hint": "啟用後，啟動時會背景檢查 GitHub 最新版本。",
        "contact_us_desc_default": "若你遇到問題、想回報錯誤，或有功能建議，歡迎透過 GitHub Issues 與我們聯繫。建議附上操作步驟與畫面資訊，協助我們更快處理。",
        "contact_us_body": "可在 GitHub Issues 開啟新議題。\n\n{desc}\n\n聯絡人：{name}\nEmail：{email}\n\nIssue 連結：\n{url}\n\n是否立即開啟？",
        "update_check_in_progress": "正在檢查更新，請稍候。",
        "update_new_version_title": "發現新版本",
        "update_new_version_body": "目前版本：{current}\n最新版本：{latest}\n\n是否開啟下載頁面？",
        "update_banner_new_version": "有新版本可用：{latest}（目前：{current}）",
        "update_banner_summary": "版本差異摘要：{summary}",
        "update_banner_rollback_hint": "可回滾提示：若升級後不符合預期，可到 Releases 下載上一版重新安裝。",
        "update_banner_latest": "目前已是最新版本（{current}）。",
        "update_banner_error": "目前無法檢查更新，請稍後再試。",
        "update_banner_checking": "正在檢查更新...",
        "update_banner_open": "立即下載",
        "update_banner_close": "稍後",
        "update_download_in_progress": "Installer download is already in progress.",
        "update_download_prompt_title": "Download update installer",
        "update_download_prompt_body": "Current version: {current}\nLatest version: {latest}\nInstaller: {installer}\n\nDownload now?",
        "update_download_progress_title": "Downloading update",
        "update_download_progress_starting": "Preparing download...",
        "update_download_progress_running_pct": "Downloading installer... {pct}%",
        "update_download_progress_bytes": "Downloaded bytes: {bytes}",
        "update_download_progress_stats": "Speed: {speed} · ETA: {eta}",
        "update_download_progress_speed_only": "Speed: {speed}",
        "update_download_cancel": "Cancel",
        "update_download_cancelling": "Cancelling download...",
        "update_download_cancelled": "Installer download cancelled.",
        "update_download_failed_title": "Download failed",
        "update_download_failed_body": "Could not download installer.\n\nDetails: {err}",
        "update_checksum_mismatch_error": "Checksum mismatch. Expected: {expected}, actual: {actual}.",
        "update_install_prompt_title": "Run installer",
        "update_install_prompt_body": "Download completed:\n{path}\n{verify_hint}\nRun installer now?",
        "update_checksum_verified_hint": "SHA256 verified: {digest}",
        "update_install_started": "Installer launched. Follow the installer steps to finish the update.",
        "update_install_failed": "Could not launch installer: {err}",
        "update_latest_title": "已是最新",
        "update_latest_body": "目前版本 {current} 已是最新版本。",
        "update_check_failed_title": "檢查更新失敗",
        "update_check_failed_body": "目前無法檢查更新，請稍後再試。",
        "open_config_path_only": "此環境無法以預設程式開啟檔案。設定檔路徑：\n{path}",
        "err_open_config_file": "無法開啟或建立設定檔：{err}",
        "analytics_title": "分析",
        "analytics_subtitle": "Matplotlib 圖表（依本機 analytics.json 累計）；下方紀錄與首頁「紀錄」同步。",
        "analytics_chart_triggers": "觸發次數",
        "analytics_range_today": "今日",
        "analytics_range_week": "最近 7 天",
        "analytics_chart_runtime": "每日運作時長",
        "analytics_chart_patterns": "路徑使用比例（累計）",
        "analytics_axis_hour": "小時（0–23）",
        "analytics_axis_day": "日期",
        "analytics_axis_count": "次數",
        "analytics_axis_runtime_min": "分鐘",
        "analytics_empty": "尚無資料 — 請開始排程並觸發微動。",
        "intro_title": "歡迎使用",
        "startup_choice_title": "啟動提示",
        "startup_lang_label": "啟動語言",
        "startup_lang_hint": "先選擇慣用語言，後續提示與教學會即時切換。",
        "startup_choice_body": "二次努力 {version}\n\n"
        "選擇是否開啟「操作流程簡介」。",
        "startup_choice_note": "你可以先看 UI 教學再開始操作；若不需要，也可直接略過並關閉後續提示。",
        "startup_choice_cta_guide": "開啟流程簡介",
        "startup_choice_cta_skip_forever": "略過並不再顯示",
        "guide_window_title": "操作流程簡介",
        "guide_features_title": "主要功能",
        "guide_features_body": "可依閒置時間自動微動、設定路徑或自然模式、排程啟停、快速鍵控制，以及更新與外觀設定。",
        "guide_step_1_title": "設定執行節奏",
        "guide_step_1_body": "在首頁控制區設定間隔（分鐘或秒）、間隔隨機與位移大小，必要時選擇路徑或自然模式。",
        "guide_step_2_title": "啟動並觀察進度",
        "guide_step_2_body": "按開始後可從頂部狀態列看倒數與排程狀態，快速確認目前是否在執行。",
        "guide_step_3_title": "使用快捷鍵與設定",
        "guide_step_3_body": "可用 F5 開始、Shift+F5 停止；在設定頁調整外觀、排程與更新選項。",
        "guide_step_counter": "步驟 {current} / {total}",
        "guide_progress_percent": "進度 {percent}%",
        "guide_cta_prev": "上一步",
        "guide_cta_next": "下一步",
        "guide_cta_done": "開始使用",
        "startup_notice_title": "用途提醒",
        "startup_notice_body": "二次努力 {version}\n\n"
        "本工具僅可用於合法且合規的生產力情境（例如簡報或閱讀時避免螢幕休眠）。\n\n"
        "禁止用於規避公司、學校或服務平台的安全、監控與稽核機制。按下「確定」以繼續。",
        "motion_pattern_log_line": "直線",
        "motion_pattern_log_circle": "圓形",
        "motion_pattern_log_square": "方形",
        "intro_body": "二次努力 {version}\n\n"
        "本程式會在鍵盤與滑鼠皆無輸入達您設定的時間後微動游標再還原：可選路徑模式（直線、圓形、方形）或自然模式（不規則微移，可選低機率點擊／捲動），可用於簡報或閱讀時避免螢幕休眠（Windows 以系統閒置時間為準）。\n\n"
        "請僅在合法、符合公司／學校規定與服務條款的前提下使用；勿用於規避安全或監控機制。\n\n"
        "按 F1 可開啟鍵盤與無障礙說明。您的偏好設定會儲存在本機，下次開啟時自動載入。\n\n"
        "點選「確定」後不會再顯示本視窗（除非刪除設定檔）。",
        "a11y_help_title": "鍵盤與無障礙說明",
        "a11y_help_body": "二次努力 {version}\n\n"
        "鍵盤：\n"
        "· F1 本說明\n"
        "· F2 / F3 / F4 首頁、設定、分析\n"
        "· F5 開始（首頁「控制面板」且可開始時）\n"
        "· Shift+F5 停止\n"
        "· Enter 開始、Esc 停止（僅主視窗顯示時；條件同 F5／Shift+F5）\n"
        "· F6 首頁切換「控制面板」/「紀錄」\n\n"
        "可點擊「間隔」「間隔隨機」「位移」「路徑速度」「移動持續時間」「活動模式」或「移動路徑」標籤，將焦點移到該欄位。間隔可選「分鐘」或「秒」；「間隔」指鍵盤與滑鼠皆無輸入需累積的時間（與主控台狀態列、Windows 系統閒置時間一致）。欄位下方有 30s／1m／5m／10m 快速按鈕。自然模式以非幾何軌跡微移游標，可選低機率點擊或捲動。\n\n"
        "注意：CustomTkinter 多數控制項以畫布繪製，部分螢幕閱讀器可能無法宣讀所有元件。視窗標題與本對話框採用系統標準外觀。",
        "dialog_btn_ok": "確定",
        "dialog_btn_yes": "是",
        "dialog_btn_no": "否",
    },
    "en": {
        "window_title": "try-working-hard",
        "app_subtitle": "try-working-hard",
        "seg_control": "Control",
        "seg_log": "Log",
        "theme_appearance": "Appearance",
        "theme_appearance_dark": "Dark",
        "theme_appearance_light": "Light",
        "theme_status_dark": "Dark theme",
        "theme_status_light": "Light theme",
        "lang_ui": "Language",
        "dashboard": "Dashboard",
        "interval_label": "Interval (sec)",
        "interval_presets_caption": "Quick presets",
        "interval_preset_10s": "10s",
        "interval_preset_30s": "30s",
        "interval_preset_1m": "1m",
        "interval_preset_5m": "5m",
        "autostart_switch_title": "Start with Windows",
        "autostart_switch_hint": "Run at sign-in and start in the system tray on supported setups.",
        "autostart_not_windows": " (Windows only.)",
        "autostart_requires_tray": " (pystray required.)",
        "log_tray_start_hidden": "Running from the tray (main window hidden).",
        "interval_unit_min": "min",
        "interval_unit_sec": "sec",
        "interval_hint_min": "≥ 0.1, decimals — min idle (no keyboard/mouse) before nudge",
        "interval_hint_sec": "5–600, decimals — min idle (no keyboard/mouse) before nudge",
        "interval_jitter_label": "Interval jitter (± sec)",
        "interval_jitter_hint": "0 = off; each idle spacing is base interval ± this many seconds · 0–{max}",
        "motion_pattern_label": "Path",
        "motion_pattern_label_when_natural": "Path shape (Pattern mode only)",
        "motion_pattern_line": "Line",
        "motion_pattern_circle": "Circle",
        "motion_pattern_square": "Square",
        "activity_style_label": "Prioritize Natural or Fixed Mode",
        "activity_style_pattern": "Fixed",
        "activity_style_natural": "Natural",
        "activity_mode_params_fixed": "Configurable parameters: interval, interval jitter, nudge size, path speed, motion duration, and path shape.",
        "activity_mode_params_natural": "Configurable parameters: interval, interval jitter, nudge size, motion speed, motion duration, occasional left click, and occasional wheel scroll.",
        "fixed_params_title": "Advanced tuning",
        "extra_params_title": "Extra parameters",
        "extra_params_expand": "Expand extra parameters",
        "extra_params_collapse": "Collapse extra parameters",
        "natural_preset_label": "Natural presets",
        "natural_preset_conservative": "Conservative",
        "natural_preset_standard": "Standard",
        "natural_preset_aggressive": "Aggressive",
        "natural_preset_load": "Load",
        "natural_preset_save": "Save",
        "natural_preset_hint": "Pick a preset and load it in one click. Save overwrites the selected preset.",
        "natural_rare_click": "Occasional left click (low rate)",
        "natural_rare_scroll": "Occasional wheel scroll (low rate)",
        "natural_intensity_label": "Natural intensity",
        "natural_intensity_conservative": "Conservative",
        "natural_intensity_standard": "Standard",
        "natural_intensity_strong": "Strong",
        "natural_intensity_tooltip_title": "Natural intensity profiles",
        "natural_intensity_tooltip_body": "Conservative: displacement 35–65%, steps 96–210, smoothing 0.90, substeps 5.\nStandard: displacement 70–95%, steps 60–140, smoothing 0.86, substeps 3.\nStrong: displacement 95–100%, steps 34–82, smoothing 0.82, substeps 3.",
        "natural_opts_hint": "Natural mode only; at most one extra action per nudge, low probability.",
        "motion_pattern_log_natural": "natural",
        "pixels_label": "Nudge (pixels)",
        "pixels_hint": "Pattern: line/circle/square as before; Natural: max wander from start · {lo}–{hi}",
        "path_speed_label": "Path speed",
        "path_speed_hint": "How fast to trace the path · {lo}–{hi} (higher = faster)",
        "path_speed_label_natural": "Motion speed",
        "path_speed_hint_natural": "Delay between natural micro-move steps · {lo}–{hi} (higher = faster)",
        "motion_duration_label": "Motion duration (sec)",
        "motion_duration_hint": "Duration per nudge in seconds · {lo}–{hi} (max 60 s)",
        "log_start_fail_motion_duration": "Start failed: invalid motion duration.",
        "err_motion_duration": "Enter a valid motion duration in seconds (integer, {lo}–{hi}).",
        "log_start_fail_path_speed": "Start failed: invalid path speed.",
        "err_path_speed": "Enter a valid path speed (integer, {lo}–{hi}).",
        "status_motion_burst": "Status: running · moving ({md} s) · ~{cd} left",
        "btn_start": "Start",
        "btn_stop": "Pause",
        "tray_checkbox": "Close to system tray (schedule continues)",
        "tray_switch_title": "Minimize to system tray on close",
        "tray_switch_hint": "When enabled, closing the window keeps the schedule running; restore from the tray icon.",
        "schedule_window_title": "Schedule window (weekdays)",
        "schedule_window_start_label": "Start",
        "schedule_window_end_label": "End",
        "schedule_window_hint": "Local 24h HH:MM; supports multi-segment windows (comma-separated, e.g. 09:00-12:00,13:00-18:00,21:00-23:00). Weekends and cron-like rules are available in config.",
        "schedule_banner_active": "Schedule: {start}–{end} local (end exclusive) · weekends: {weekend} · cron: {cron}",
        "schedule_banner_need_valid_times": "Schedule on: enter valid windows as HH:MM-HH:MM (comma-separated allowed, no overlaps).",
        "err_schedule_bounds": "Enter a valid schedule: HH:MM-HH:MM windows (comma-separated, non-overlapping); cron-like expressions must also be valid if set.",
        "log_start_fail_schedule_bounds": "Start failed: invalid schedule window times.",
        "status_schedule_wait": "Status: schedule paused · resuming in ~{cd}",
        "log_start_schedule": "Schedule enabled: base window {start}–{end} (local), weekends: {weekend}, cron: {cron}.",
        "log_schedule_resumed": "Back within work hours; schedule continues.",
        "schedule_weekend_on": "on",
        "schedule_weekend_off": "off",
        "schedule_cron_off": "none",
        "tray_no_pystray": "(pystray not installed)",
        "status_stopped": "Status: stopped",
        "status_running_min": "Status: running · nudge after {v:g} min idle (keyboard/mouse) · next in {cd}",
        "status_running_sec": "Status: running · interval {v:g} s · motion duration {md} s · next in {cd}",
        "log_title": "Activity log",
        "log_ready": "Ready.",
        "log_start_fail_interval": "Start failed: invalid interval.",
        "log_start_fail_pixels": "Start failed: invalid nudge size.",
        "log_started_min": "Started: nudge after {v:g} min (~{sec:.0f} s) with no keyboard/mouse input, path {pat}, nudge {px} px, path speed {ps}, motion duration {md} s.",
        "log_started_min_jitter": "Started: nudge after ~{v:g} min base (~{sec:.0f} s idle) with ±{j:g} s jitter between waits, path {pat}, nudge {px} px, path speed {ps}, motion duration {md} s.",
        "log_started_sec": "Started: nudge after {v:g} s with no keyboard/mouse input, path {pat}, nudge {px} px, path speed {ps}, motion duration {md} s.",
        "log_started_sec_jitter": "Started: nudge after ~{v:g} s base idle with ±{j:g} s jitter between waits, path {pat}, nudge {px} px, path speed {ps}, motion duration {md} s.",
        "log_stopped": "Stopped manually.",
        "log_nudge": "Cursor nudge executed.",
        "log_nudge_zero": "Tick fired (0 px — cursor not moved).",
        "log_nudge_fail": "Nudge failed: {err}",
        "log_exit": "Exiting.",
        "log_tray_minimize": "Minimized to tray (schedule still running).",
        "err_title": "Invalid input",
        "err_minutes": "Enter a valid interval in minutes (number, ≥ {min}).",
        "err_seconds": "Enter a valid interval in seconds (number, {min}–{max}).",
        "err_jitter": "Enter a valid jitter in seconds (number, 0–{max}).",
        "log_start_fail_jitter": "Start failed: invalid interval jitter.",
        "err_pixels": "Enter a valid nudge size (integer, {lo}–{hi}).",
        "tray_show": "Show window",
        "tray_quit": "Quit",
        "nav_home": "Home",
        "nav_settings": "Settings",
        "nav_analytics": "Analytics",
        "settings_title": "Settings",
        "btn_open_config_file": "Open config file",
        "about_updates_title": "About and updates",
        "version_info": "Version: {version}",
        "btn_contact_us": "Contact us",
        "btn_check_updates": "Check for updates",
        "auto_check_updates_title": "Auto-check updates on startup",
        "auto_check_updates_hint": "When enabled, the app checks GitHub releases in the background at startup.",
        "contact_us_desc_default": "If you found a bug or have a feature suggestion, please contact us through GitHub Issues. Including reproduction steps and screenshots will help us resolve it faster.",
        "contact_us_body": "You can open a new issue on GitHub Issues.\n\n{desc}\n\nContact: {name}\nEmail: {email}\n\nIssue link:\n{url}\n\nOpen now?",
        "update_check_in_progress": "Update check is already running.",
        "update_new_version_title": "Update available",
        "update_new_version_body": "Current version: {current}\nLatest version: {latest}\n\nOpen the download page now?",
        "update_banner_new_version": "New version available: {latest} (current: {current})",
        "update_banner_summary": "Version diff summary: {summary}",
        "update_banner_rollback_hint": "Rollback tip: if the upgrade is not ideal, download the previous release from Releases and reinstall.",
        "update_banner_latest": "You are already on the latest version ({current}).",
        "update_banner_error": "Could not check updates right now. Please try again later.",
        "update_banner_checking": "Checking for updates...",
        "update_banner_open": "Download now",
        "update_banner_close": "Later",
        "update_download_in_progress": "Installer download is already in progress.",
        "update_download_prompt_title": "Download update installer",
        "update_download_prompt_body": "Current version: {current}\nLatest version: {latest}\nInstaller: {installer}\n\nDownload now?",
        "update_download_progress_title": "Downloading update",
        "update_download_progress_starting": "Preparing download...",
        "update_download_progress_running_pct": "Downloading installer... {pct}%",
        "update_download_progress_bytes": "Downloaded bytes: {bytes}",
        "update_download_progress_stats": "Speed: {speed} · ETA: {eta}",
        "update_download_progress_speed_only": "Speed: {speed}",
        "update_download_cancel": "Cancel",
        "update_download_cancelling": "Cancelling download...",
        "update_download_cancelled": "Installer download cancelled.",
        "update_download_failed_title": "Download failed",
        "update_download_failed_body": "Could not download installer.\n\nDetails: {err}",
        "update_checksum_mismatch_error": "Checksum mismatch. Expected: {expected}, actual: {actual}.",
        "update_install_prompt_title": "Run installer",
        "update_install_prompt_body": "Download completed:\n{path}\n{verify_hint}\nRun installer now?",
        "update_checksum_verified_hint": "SHA256 verified: {digest}",
        "update_install_started": "Installer launched. Follow the installer steps to finish the update.",
        "update_install_failed": "Could not launch installer: {err}",
        "update_latest_title": "Up to date",
        "update_latest_body": "You are already on the latest version ({current}).",
        "update_check_failed_title": "Update check failed",
        "update_check_failed_body": "Could not check updates right now. Please try again later.",
        "open_config_path_only": "This platform has no default “open with” for files. Config path:\n{path}",
        "err_open_config_file": "Could not open or create the config file: {err}",
        "analytics_title": "Analytics",
        "analytics_subtitle": "Matplotlib charts from local aggregates (analytics.json). Log stays synced with Home → Log.",
        "analytics_chart_triggers": "Nudge counts",
        "analytics_range_today": "Today",
        "analytics_range_week": "Last 7 days",
        "analytics_chart_runtime": "Scheduled uptime per day",
        "analytics_chart_patterns": "Path usage (totals)",
        "analytics_axis_hour": "Hour (0–23)",
        "analytics_axis_day": "Date",
        "analytics_axis_count": "Count",
        "analytics_axis_runtime_min": "Minutes",
        "analytics_empty": "No data yet — start the schedule and generate nudges.",
        "intro_title": "Welcome",
        "startup_choice_title": "Startup prompt",
        "startup_lang_label": "Startup language",
        "startup_lang_hint": "Pick your preferred language first; prompts and onboarding copy update immediately.",
        "startup_choice_body": "try-working-hard {version}\n\n"
        "Choose whether to open the operation flow guide.",
        "startup_choice_note": "You can preview a guided UI walkthrough before using the app, or skip and disable this prompt.",
        "startup_choice_cta_guide": "Open quick guide",
        "startup_choice_cta_skip_forever": "Skip and do not show again",
        "guide_window_title": "Operation flow quick guide",
        "guide_features_title": "Core capabilities",
        "guide_features_body": "Idle-aware nudging, pattern or natural activity modes, schedule windows, shortcut controls, and update/theme preferences.",
        "guide_step_1_title": "Configure run profile",
        "guide_step_1_body": "In Home → Control, set interval (min/sec), optional jitter, nudge size, and choose Pattern or Natural activity.",
        "guide_step_2_title": "Start and monitor status",
        "guide_step_2_body": "Press Start and monitor countdown plus schedule state in the top status strip.",
        "guide_step_3_title": "Shortcuts and settings",
        "guide_step_3_body": "Use F5 to start, Shift+F5 to stop, and open Settings for appearance, schedule, and update preferences.",
        "guide_step_counter": "Step {current} / {total}",
        "guide_progress_percent": "Progress {percent}%",
        "guide_cta_prev": "Previous",
        "guide_cta_next": "Next",
        "guide_cta_done": "Start using app",
        "startup_notice_title": "Usage reminder",
        "startup_notice_body": "try-working-hard {version}\n\n"
        "Use this tool only for lawful and policy-compliant productivity scenarios (for example, keeping the screen awake while presenting or reading).\n\n"
        "Do not use it to bypass company, school, or service security, monitoring, or audit controls. Click OK to continue.",
        "motion_pattern_log_line": "line",
        "motion_pattern_log_circle": "circle",
        "motion_pattern_log_square": "square",
        "intro_body": "try-working-hard {version}\n\n"
        "This app nudges the cursor only after the interval elapses with no keyboard or mouse input, then restores it. Choose Pattern mode (line, circle, or square) or Natural mode (irregular micro-moves with optional low-rate click or scroll)—useful to keep the screen awake while presenting or reading (on Windows, idle time uses the system last-input time).\n\n"
        "Use it only in lawful ways that comply with employer, school, and service rules; do not use it to bypass security or monitoring.\n\n"
        "Press F1 for keyboard and accessibility help. Your preferences are saved locally and loaded next time.\n\n"
        "You will not see this dialog again after you click OK (unless you delete the config file).",
        "a11y_help_title": "Keyboard and accessibility",
        "a11y_help_body": "try-working-hard {version}\n\n"
        "Keyboard:\n"
        "· F1  this help\n"
        "· F2 / F3 / F4  Home, Settings, Analytics\n"
        "· F5  Start (Home → Control, when available)\n"
        "· Shift+F5  Stop\n"
        "· Enter / Esc  Start / Stop (main window visible only; same rules as F5 / Shift+F5)\n"
        "· F6  Home: toggle Control / Log\n\n"
        "Click the Interval, Interval jitter, Nudge (pixels), Path speed, Motion duration (sec), Activity, or Path label to move focus to that field. Choose min or sec for the interval: Interval is the required idle time with no keyboard or mouse input before a nudge (status line uses the same idle time as Windows GetLastInputInfo). Use 30s / 1m / 5m / 10m under the field for a quick set. Natural mode uses non-geometric micro-moves; optional click and scroll fire rarely.\n\n"
        "Note: CustomTkinter draws most controls on a canvas, so not every control is exposed to all screen readers. The window title and this dialog use standard toolkit UIs.\n"
        "Tab / Shift+Tab move focus; tooltips are not used for the canvas controls.",
        "dialog_btn_ok": "OK",
        "dialog_btn_yes": "Yes",
        "dialog_btn_no": "No",
    },
}
